...
export default function PodcastPage() {
  ...
  return (
    <div className="px-4 py-4 sm:px-6 lg:px-8">
      <h1 className="text-4xl font-bold">پادکست</h1>
      <p className="mt-2 text-lg text-[#4a4a4a]">قسمت‌های پادکست نیلوفر.</p>
      ...
      <div className="mt-5">
        {episodes.length > 0 ? (
              ...
              return (
                ...
                >
                  <div className="hairline-cell flex flex-1 flex-col">
                    <p className="text-sm text-[#6b6b6b]">{formatJalaliDate(ep.date)}</p>
                    <h3 className="mt-1 text-xl font-bold transition duration-200 group-hover:text-[var(--accent)]">
                      {ep.title}
                    </h3>
                    <p className="mt-3 text-[17px] leading-8 text-[#333333]">{ep.description}</p>
                  </div>
                </a>
              );
            })}
          ...
          <div className="border border-dashed border-[var(--hairline)] py-16 text-center text-[#6b6b6b]">
            ...
            <p className="mt-2 text-base">قسمت‌های پادکست نیلوفر به‌زودی اینجا منتشر می‌شود.</p>
          </div>
        )}
      </div>
    </div>
